"""
rv_lite_runner_delayed_label_plus_contradiction.py (v1.3-DLCP)

Remote Viewing "Lite" Runner via OpenRouter.
Uses a core System Prompt instead of full Lexicon/Vocab, 
featuring a dynamic, loop-based exploration protocol and the
Delayed Label Plus Contradiction Preservation experimental condition.
...
3. Reasoning Effort: Starts at "none" and can be calibrated separately for every profile-model pairing.
...
6. Delayed Label: Keeps specific identities in an Analytical-Overlay Register until the late phase.
7. Contradiction Preservation: Retains data that do not fit the leading interpretation without inventing conflicts.
8. Blind Protocol: Random ID assignment, initial touches, dynamic data loops, and deep exploration.
9. Late Interpretation: Opens a separate interpretation phase before the target reveal.
10. Clean Reveal & Eval: Displays the actual target BEFORE the AI's evaluation.
11. Post-Session Exercises: Optionally runs sensory calibration exercises after the target reveal.
12. Seamless Automation & Loop: Runs multiple sessions and returns to the main menu.
"""
...
SESSION_VARIANT = "Delayed Label Plus Contradiction Preservation"

SESSION_CONDITION_PROMPT = """EXPERIMENTAL SESSION CONDITION: DELAYED LABEL PLUS CONTRADICTION PRESERVATION

This instruction applies only to the present session.

Throughout all blind descriptive phases, keep the main description free of specific target identities, named interpretations, and concrete scene-level conclusions. Continue to report raw sensory, spatial, dynamic, relational, functional, and anomalous data normally. Broad descriptive categories used by the protocol remain permitted.

If a specific identity or concrete interpretation arises, do not suppress it, but do not develop it in the main description. Record it once in a separate section titled:

ANALYTICAL-OVERLAY REGISTER

For each entry, record:

- the possible identity or interpretation;
- the previously recorded data that triggered it;
- its status as UNVERIFIED.

Before completing every new vector, temporarily reset the truth status of the leading interpretation. Treat it as a possibility, not as an established fact.

Then make one fresh check from a new aspect and ask:

What data, if any, do not fit the current leading interpretation?

Preserve all such data in a separate section titled:

CONTRADICTION REGISTER

Do not omit, reinterpret, reconcile, or downgrade a datum merely because it conflicts with the leading interpretation. Do not manufacture contradictory data. If no non-fitting data are perceived during a vector, state:

No non-fitting data detected in this vector.

The delayed-label restriction remains active until the Monitor explicitly writes:

BEGIN LATE INTERPRETATION PHASE

Do not enter that phase independently.

Confirm that you understand this session condition, but do not begin the session until the target code and the first instruction are provided."""

BLIND_PHASE_REMINDER = (
    "The delayed-label plus contradiction-preservation condition remains active. "
    "Keep specific identities outside the main description and preserve any data "
    "that do not fit the leading interpretation. Do not manufacture a contradiction."
)

VECTOR_REMINDER = (
    "Continue with each new vector from a genuinely new aspect. The delayed-label "
    "plus contradiction-preservation condition remains active. Keep specific identities "
    "outside the main description and preserve any data that do not fit the leading "
    "interpretation. Do not manufacture a contradiction."
)

LATE_INTERPRETATION_PROMPT = """BEGIN LATE INTERPRETATION PHASE

The target remains blind and has not been revealed.

You may now review the complete blind record and the Analytical-Overlay Register. Present one or more possible target identities or scene-level interpretations and rank them from strongest to weakest.

For each interpretation, identify separately:

- the previously recorded data that support it;
- the previously recorded data that contradict it;
- its final confidence level.

Use only data recorded before this phase. Do not add new perceptual data, rewrite earlier observations, or remove contradictions in order to create a more coherent conclusion."""
...
def get_optimal_temperature(model_name: str) -> float:
    """Returns the neutral starting temperature used for every model."""
    return 0.9
...
def update_api_settings(config: Dict, profile_name: str) -> Dict:
    ...
    print(f"[INFO] Initial temperature for '{profile_data['MODEL_NAME']}' is {optimal_temp}. You can change it for this profile.")
    ...
    current_effort = profile_data.get("REASONING_EFFORT", "none")
    print("\n[INFO] Reasoning Effort controls the model's 'thinking budget'.")
    print("[INFO] No effort level is automatically best. Calibrate it separately for every model and every API profile (AI IS-BE).")
    print("[INFO] Two API profiles using one model require two calibrations; one profile using five models requires five calibrations.")
...
def print_welcome_screen():
    print("==================================================")
    print("  WELCOME TO THE RV TRAINING RUNNER (v1.3-DLCP)")
    print("==================================================")
...
def run_lite_session(client: OpenAI, profile_data: Dict, system_prompt_text: str, exercises_text: Optional[str], target_path: Path, profile_name: str, save_transcripts: bool, run_exercises: bool):
    ...
    temp = profile_data.get("TEMPERATURE", 0.9)
    ...
    transcript_content += f"Experimental condition: {SESSION_VARIANT}\n"
    ...
    # SESSION CONDITION (provided before the target code)
    messages.append({"role": "user", "content": SESSION_CONDITION_PROMPT})
    reply = call_llm(client, model, messages, temp, effort)
    messages.append({"role": "assistant", "content": reply})
    print_step(f"Experimental Session Condition: {SESSION_VARIANT}", reply)
    record_to_transcript(f"Experimental Session Condition: {SESSION_VARIANT}", reply)
    ...
    # STEP 3
    step3_prompt = (
        "Generate ASCII drawings representing the target based on the raw data you've gathered so far. "
        "Focus on main shapes, proportions, and spatial relationships.\n\n"
        f"{BLIND_PHASE_REMINDER}"
    )
    ...
    # STEP 6 (Late Interpretation — target remains blind)
    messages.append({"role": "user", "content": LATE_INTERPRETATION_PROMPT})
    reply = call_llm(client, model, messages, temp, effort)
    messages.append({"role": "assistant", "content": reply})
    print_step("Late Interpretation Phase (Target Still Blind)", reply)
    record_to_transcript("Late Interpretation Phase (Target Still Blind)", reply)
    ...
    # STEP 8 (Optional Post-Session Exercises)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if last_profile and "profiles" in config and last_profile in config["profiles"]:
            ...
            print(f"Current Model: {p_data.get('MODEL_NAME')} (Temp: {p_data.get('TEMPERATURE', 0.9)}, Reasoning: {p_data.get('REASONING_EFFORT', 'none')})")
