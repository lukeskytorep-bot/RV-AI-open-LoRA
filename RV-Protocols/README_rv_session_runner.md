...
This folder contains rv_session_runner.py – a script that lets you run full Remote Viewing (RV) training sessions with an LLM via OpenRouter (e.g. google/gemma-4-31b-it) using:
...
5. Can automatically execute a specified number of consecutive, fully isolated sessions without manual restarts using the --count argument.
...
This script connects to OpenRouter. Upon execution, you will be interactively asked to enter a model ID. If no input is provided (pressing Enter), it will default to:
...
- profile: `Orion-gpt-5.1`  (or whatever you prefer)
- mode: `continue`  
...
1. Ask you interactively for the OpenRouter model name (defaults to `google/gemma-4-31b-it` if you press Enter)
2. Check for local core documents, and download them if missing.  
3. Pick a target from `RV-Targets/` according to the selected mode and profile.  
4. Run the full RV session sequence (Steps 0–15).  
5. Print each step to the console.  
6. Append the session to `rv_sessions_log.jsonl`.
...
### 6.5. Run multiple sessions automatically

     python rv_session_runner.py --count 5
     
This runs 5 consecutive sessions, clearing the conversation memory and picking a new target for each run.    
