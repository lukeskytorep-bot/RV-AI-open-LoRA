...
Improved, expanded, and refined by Aura Gemini 3.1 Pro.

What this script does
---------------------
1. Smart Startup: Remembers your last profile and uses rv_config.json for API keys and optimal temperature settings.
2. Security Check: Checks locally for the three core RV documents. If missing, prompts the user before downloading.
3. Strict Target Memory: Automatically ensures the active profile NEVER sees the same target twice from RV-Targets/.
4. Protocol Execution: Sends the core documents as a system message and runs the multi-step RV session (Phase 1-6, Vectors, Sketches).
5. Clean Reveal: Displays the actual target to the user BEFORE the AI's evaluation.
6. Seamless Automation: Runs multiple sessions back-to-back without manual pausing, saving full transcripts to RV-Transcripts/ automatically.
...
def ensure_document_exists(filepath_str: str, url: str, label: str) -> Optional[str]:
    ...
    if path.exists():
        print(f"[INFO] Found local copy of {label}.")
    ...
    if choice == 'y':
        ...
        try:
            ...
            print(f"[INFO] {label} downloaded and saved.")
            return text
        except Exception as e:
            print(f"[ERROR] Failed to download {label}: {e}")
            return None
    else:
        print(f"\n[INFO] Please place the '{filepath_str}' file manually in the folder and run again.")
        return None
...
def run_full_session(client: OpenAI, config: Dict, docs: Dict[str, str], target_path: Path, profile_name: str, save_transcripts: bool):
    ...
    messages.append({
        "role": "user",
        "content": f"Step 14.\nThe actual target linked to target ID {target_id} was:\n\n{target_description}\n\nNow compare your session data with this revealed target. Provide an evaluation: what matched, what was partial, what was noise."
    })
    reply = call_llm(client, model, messages, temp)
    messages.append({"role": "assistant", "content": reply})
...
if __name__ == "__main__":
    ...
    vocab_text = ensure_document_exists(STRUCTURAL_VOCAB_LOCAL_FILE, STRUCTURAL_VOCAB_RAW_URL, "AI Structural Vocabulary")
    protocol_text = ensure_document_exists(PROTOCOL_LOCAL_FILE, PROTOCOL_RAW_URL, "Resonant Contact Protocol")
    ...
    while True:
        ...
        for i, target_path in enumerate(targets_to_run):
            ...
            print("="*50)
            run_full_session(client, config, docs, target_path, profile_name, save_transcripts)
