"""
rv_quadruple_session_runner.py (v2.2)

Remote Viewing "Quadruple-Blind" Runner via OpenRouter.
This advanced script runs a quadruple-blind protocol on a single target. 
It initiates 4 separate sessions on the SAME target, each with a different
level of Reasoning Effort (none, low, medium, high).
The order of Reasoning Efforts is RANDOMIZED for every new target to eliminate Order Bias.
Memory is frozen between runs, and target reveals happen in reverse chronological order 
(Session 4, then 3, then 2, then 1) for evaluation.
...
LOG_FILE = "rv_quadruple_sessions_log.jsonl"
...
def print_welcome_screen():
    print("=======================================================")
    print("    WELCOME TO THE RV QUADRUPLE-SESSION RUNNER (v2.2)")
    print("=======================================================")
    print("This script executes an advanced Quadruple-Session protocol.")
    print("The AI performs FOUR independent sessions on the SAME target,")
    print("covering the full spectrum of Reasoning Effort (None, Low, Medium, High).")
    print("The order of the efforts is RANDOMIZED for each target to eliminate bias.")
    print("Memory is frozen between runs, and target reveals happen")
    print("in reverse chronological order to verify reasoning depth impact.")
    ...
    print("- Reasoning Effort 'high' may consume a massive amount of tokens.")
    print("=======================================================\n")
...
def run_quadruple_blind_session(client: OpenAI, profile_data: Dict, system_prompt_text: str, exercises_text: Optional[str], target_path: Path, profile_name: str, save_transcripts: bool, run_exercises: bool):
    ...
    def process_evaluation_and_exercises(msgs: List[Dict], trans: str, t_id: str, label: str, current_effort: str):
        ...
        if save_transcripts:
            ...
            transcript_path = Path(TRANSCRIPTS_DIR) / f"QuadrupleSession_{label}_{current_effort.upper()}_{t_id}_{profile_name}.txt"
    ...
    # ────────────────────────────────────────────────────────
    # DYNAMICZNA PĘTLA SESJI (LOSOWY REASONING)
    # ────────────────────────────────────────────────────────
    efforts = ["none", "low", "medium", "high"]
    random.shuffle(efforts)

    print(f"\n[INFO] Randomized Reasoning Effort order for this target: {', '.join(e.upper() for e in efforts)}")
    ...
    for i, effort in enumerate(efforts):
        ...
        print(f"\n[INFO] === FULL-SPECTRUM SESSION: RUN {label} OF 4 (Reasoning: {effort.upper()}) ===")
    ...
    for res in reversed(session_results):
        if res["label"] == "4":
            print(f"\n[INFO] Evaluating Session 4 (Reasoning: {res['effort'].upper()})...")
        else:
    ...
    print(f"\n[INFO] Quadruple-Blind Target Processing Completed.")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if last_profile and "profiles" in config and last_profile in config["profiles"]:
            ...
            print("[INFO] Reasoning Effort will be handled automatically (randomized order for each target).")
            ...
            print(" [S] Change API SETTINGS for current profile (Key, Model, Temp)")
            print(" [Q] Quit")
        ...
        # Target count
        try:
            count_input = input("How many targets would you like to process with the Quadruple-Blind protocol? (default 1): ").strip()
        ...
        print(f"\n[INFO] Initializing batch of {actual_run_count} quadruple-blind processing routines...")
        for i, target_path in enumerate(targets_to_run):
            print(f"\n" + "="*50)
            print(f"[INFO] PROCESSING TARGET {i+1} OF {actual_run_count}")
            print("="*50)
            run_quadruple_blind_session(client, profile_data, system_prompt, exercises_text, target_path, profile_name, save_transcripts, run_exercises)
